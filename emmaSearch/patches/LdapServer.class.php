...
/**
 * Active Directory Patch for password attribute 'unicodePwd'
 *
 * Saving password is required in 2 occassions: 
 * (1) when a new user is created: via createLdapEntry()
 * (2) when existing user changes password: via modifyLdapEntry()
 *
 * This is done via encryptPassword()
 *
 */
 ...
 */
  ...
  public function createLdapEntry($attributes, $dn = NULL) {
    ...
    // You need to set this attribute in ldap_users module interface

    foreach ($attributes as $key => $cur_val) {        
        if ($key == "unicodePwd") {           
            $attributes[$key][0] = $this->encryptPassword($cur_val[0]);          
        }  
    } 
 ...
 */
  ...
  function modifyLdapEntry($dn, $attributes = array(), $old_attributes = FALSE) {
    ...
    if (count($attributes) > 0) {
        // AD Patch: need to make sure password is 128-bit encrypted for AD
        foreach ($attributes as $key => $cur_val) {        
            if ($key == "unicodePwd") {           
                $attributes[$key] = $this->encryptPassword($cur_val);          
            }  
        }         
